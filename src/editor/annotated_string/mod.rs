...
impl AnnotatedString {
    ...
    pub fn truncate_right_until(&mut self, from: ByteIdx) {
    ...
    pub fn replace(&mut self, start: ByteIdx, end: ByteIdx, new_string: &str) {
        ...
        // No adjustment of annotations needed in case the replacement did not result in
        // a change in length.
        if len_difference == 0 {
            return;
        }
        ...
        self.annotations.iter_mut().for_each(|annotation| {
            annotation.start = if annotation.start >= end {
                // For annotations starting after the replaced range, we move the start index by
                ...
                // For annotations starting within the replaced range, we move the start index
                // by the difference in length, constrained to the beginning or end of the
                // replaced range.
                if shortened {
            ...
            annotation.end = if annotation.end >= end {
                // For annotations ending after the replaced range, we move the end index by the
                ...
                // For annotations ending within the replaced range, we move the end index by
                // the difference in length, constrained to the beginning or end of the replaced
                // range.
                if shortened {

...
impl Line {
    ...
    pub fn get_visible_graphemes(&self, range: Range<ColIdx>) -> String {
        self.get_annotated_visible_sub_str(range, None, None)
            .to_string()
    }
    ...
    // - selected_match: The selected match to highlight on the annotated string.
    //   This is only applied if the query is not empty
    pub fn get_annotated_visible_sub_str(
        &self,
        range: Range<ColIdx>,
        query: Option<&str>,
        ...
        self.string.chars().enumerate().for_each(|(idx, ch)| {
            if ch.is_ascii_digit() {
                result.add_annotation(AnnotationType::Dight, idx, idx.saturating_add(1));
            }
        });
        ...
        if let Some(query) = query {
            if !query.is_empty() {
                self.find_all(query, 0..self.string.len()).iter().for_each(
                    |(start, grapheme_idx)| {
                        if let Some(select_match) = selected_match {
                            if *grapheme_idx == select_match {
        ...
        // We do this backwards, otherwise the byte indices would be off in case a
        // replacement character has a different width than the original character.
        ...
        for fragment in self.fragments.iter().rev() {
            ...
            if fragment_start < range.end && fragment_end > range.end {
                ...
                result.truncate_right_until(fragment.start.saturating_add(fragment.grapheme.len()));
                continue;
            }

            // Fragment ends at the start of the range: Remove the entire left side of the
            // string (if not already ata start of string)
            if fragment_end <= range.start {
                result.truncate_left_until(fragment.start.saturating_add(fragment.grapheme.len()));
                // End processing since all remaining fragments wil be invisible.
                break;
            } else if fragment_start < range.start && fragment_end > range.start {
                // Fragment overlaps with the start of range: Remove the left side of the string
                // and add an ellipsis
                result.replace(
                    0,
                ...
                // End processing since all remaining fragments will be
                // invisible.
            }
    ...
    // Inserts a character into the line, or appends it at the end if at ==
    // grapheme_count + 1
    ...
    fn find_all(&self, query: &str, range: Range<ByteIdx>) -> Vec<(ByteIdx, GraphemeIdx)> {
        ...
        self.string
            .get(start..end)
            .map_or_else(Vec::new, |sub_str| {
                let potential_matches: Vec<ByteIdx> = sub_str
                    .match_indices(query)
